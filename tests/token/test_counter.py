...
from src.token.counter import DEFAULT_IMAGE_TOKEN_COUNT, TokenCounter, TokenizerType, _process_messages
...
@pytest.fixture
def mock_token_counter(mock_logger):
...
def test_select_llama_tokenizer(mock_token_counter):
    mock_token_counter._select_tokenizer_helper.cache_clear()
...
def test_openai_token_counting_with_tools(mock_token_counter):
    ...
    with patch("src.token.counter._format_function_definitions") as mock_format:
        ...
        num_tokens = mock_token_counter._openai_token_counter(
            model="gpt-4", messages=[{"role": "system", "content": "test"}], tools=tools
        )
        assert mock_format.called
        assert num_tokens > 9  # Verify tool tokens added


def test_token_counter_with_huggingface(mock_token_counter):
    with patch("src.token.counter.Tokenizer") as mock_tokenizer:
        mock_encoder = Mock()
        mock_encoder.encode.return_value.ids = [1, 2, 3]
        mock_token_counter._select_tokenizer_helper = Mock(
            return_value=Mock(tokenizer_type=TokenizerType.HuggingFace, tokenizer=mock_encoder)
        )
        result = mock_token_counter.token_counter(model="llama-3", text="test")
        assert result == 3 + 0  # 3 text tokens + 0 image tokens


def test_error_when_no_input(mock_token_counter):
    with pytest.raises(ValueError):
        mock_token_counter.token_counter(model="gpt-4")


def test_tool_choice_handling(mock_token_counter):
    num_tokens = mock_token_counter._openai_token_counter(
        model="gpt-4", tool_choice={"function": {"name": "test_tool"}}
    )
    assert num_tokens >= 7  # Minimum tokens for tool choice


def test_caching_mechanism(mock_token_counter):
    mock_token_counter._select_tokenizer_helper.cache_clear()
    with patch("src.token.counter.Tokenizer.from_pretrained") as mock_from_pretrained:
        mock_token_counter._select_tokenizer_helper("llama-3-8b")
        mock_token_counter._select_tokenizer_helper("llama-3-8b")
        actual_call = mock_from_pretrained.call_count
        assert actual_call == 1


def test_fallback_to_default_encoding(mock_token_counter, mock_logger):
...
def test_openai_token_counter_simple(mock_token_counter):
    model = "gpt-4o"
    messages = [
        {
            "role": "user",
...
def test_openai_token_counter_without_model(mock_token_counter):
    messages = [
        {
            "role": "user",
            "content": [
...
def test_openai_token_counter_with_image(mock_token_counter):
    model = "gpt-4o"
    messages = [
        {
            "role": "user",
...
def test_openai_token_counter_with_tool(mock_token_counter):
    model = "gpt-4o"
    messages = [
        {
            "role": "user",
...
def test_openai_token_counter_with_tool_system_msg(mock_token_counter):
    model = "gpt-4o"
    messages = [
        {
            "role": "system",
