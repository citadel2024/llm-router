...
def test_select_provider_with_no_providers(mock_balancer):
    result = mock_balancer.schedule_provider("test-group", [], text="text")
    assert result is None


def test_select_provider_no_usage_data(mock_balancer, mock_providers, mock_cache):
...
def test_get_usage_data(mock_balancer, mock_providers):
...
def test_find_optimal_provider(mock_balancer, mock_providers, current_tpm, input_tokens, expected_id):
    usage_data = {
...
def test_select_lowest_tpm(mock_balancer, mock_providers, mock_cache):
...
def test_select_from_one_candidate(mock_balancer, mock_providers, mock_cache):
    tpm_data = {"model-1": 30, "model-2": 31}
...
def test_select_from_empty_rpm(mock_balancer, mock_cache):
    providers = [
...
def test_select_from_empty_tpm(mock_balancer, mock_cache):
    providers = [
