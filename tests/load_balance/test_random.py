...
def test_schedule_provider_empty_providers(mock_dependencies):
    """Test scheduling when no healthy providers are available."""
    lb_cache, log_cfg, load_balancer_config = mock_dependencies
    balancer = RandomBalancer(lb_cache, log_cfg, load_balancer_config)

    result = balancer.schedule_provider("test_group", [])
    assert result is None


def test_schedule_provider_single_provider(mock_dependencies):
    """Test scheduling when only one provider is available."""
    lb_cache, log_cfg, load_balancer_config = mock_dependencies
    balancer = RandomBalancer(lb_cache, log_cfg, load_balancer_config)

    providers = [{"name": "provider1", "endpoint": "http://test1.com"}]
    result = balancer.schedule_provider("test_group", providers)

    assert result == providers[0]


def test_schedule_provider_multiple_providers(mock_dependencies):
...
def test_schedule_provider_with_messages(mock_dependencies):
    ...
    providers = [{"name": "provider1", "endpoint": "http://test1.com"}]
    messages = [
        ChatCompletionUserMessage(role="user", content="Test message")
    ]

    result = balancer.schedule_provider("test_group", providers, messages)
